...
`HLE`（`Humanity's Last Exam`）是一张面向前沿模型的超高难度闭卷 benchmark：题目由专家编写，覆盖跨学科与部分多模态场景，主要关注模型在接近人类专家边界时能否稳定推理并给出可自动判分的答案。
...
- 它测的是 frontier 模型在**专家级闭合题**上的上限，难度定位明显高于日常知识问答。
...
- 题面短，但信息密度高
- 常带专业名词、公式、图像或学科背景
- 最终答案是可校验的短结果，长度和格式通常收敛得更紧

这和常规知识 benchmark 的差异在于：

- 难点主要来自推理链和学科深度
- 题目通常需要比 4 选 1 或简单检索更长的推理链
- 出题时已经预先考虑“能否自动打分”
...
这意味着 HLE 属于闭合式高难题 benchmark，不覆盖开放式 essay 写作。

### 4.4 数据是怎么做出来的

官方公开材料强调了三件事：

1. 题目由广泛的学科专家贡献，非自动生成。
...
也正因此，它和 `MMLU-Pro`、`GPQA` 形成递进关系，继续往更硬的位置推进。
