...
`WMT24++` 是一个面向多语言机器翻译的大规模 benchmark，它不是简单地看英法英德这种传统高资源对，而是把真实文档、更多语言方向和统一自动指标放进同一评测框架里，看模型跨语言翻译到底稳不稳。
...
- 它测的是翻译质量，不是知识保持，也不是指令遵循。
- 解释分数时必须记住：自动 MT 指标很有用，但不是人工偏好或业务可用性的完整替代。
...
这让它比只测少数主流语对的 MT benchmark 更像真实多语产品场景。
