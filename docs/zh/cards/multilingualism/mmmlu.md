...
`MMMLU` 是把原版 `MMLU` 测试集用专业人工翻译到 14 种语言后的多语种 benchmark，用来测试模型的**跨语言知识保持**。它不用于判断表达是否更自然。
...
高分主要说明跨语言知识保持较好，不足以证明模型真正理解本地文化语境；题目也并非各语言本地编写。
...
> `MMMLU` 值得保留，但它更适合作为“多语言知识迁移基线”。完整的多语智能评估仍需要搭配原生多语任务一起看。
